...
$(document).ready(function(){
    ...
    $(".add-more").click(function(e){
        ...
        next = next + 1;
        var newIn = '<div id="field'+next +'"><input autocomplete="off" class="input form-control noob" id="src' + next + '" name="src' + next + '" type="text" placeholder="Source"><input autocomplete="off" class="input form-control noob" id="dest' + next + '" name="dest' + next + '" type="text" placeholder="Destination"><input type="checkbox" class="check noob" id="spec'+next+'" name="spec'+next+'" value='+next+'><button id="remove' + (next) + '" class="btn btn-danger remove-me noob" >X</button></div>';
        var newInput = $(newIn);
        ...
        $("#count").val(next);
            ...
            $('.remove-me').click(function(e){
                ...
                var srcID = "#src" + fieldNum;
                var destID = "#dest" + fieldNum;
                var specID = "#spec"+ fieldNum;
                $(this).remove();
                $(srcID).remove();
                $(destID).remove();
                $(specID).remove();
...
// {
//    var mymap = L.map('mapid').setView([40.7403,-73.9839], 16);
...
//
// function dat_inp() {
//     console.log("dat_inp is working!") ;// sanity check
//     $.ajax({
//         url : "dat_inp/", // the endpoint
//         type : "POST", // http method
//         data : { src : $('#src1').val(), dest:  $('#dest1').val()}, // data sent with the post request
//
...
$(document).on('submit', '#post-form',function(e){
    ...
    $.ajax({
        ...
        success:function(json){
         ...
         col_arr = ['#4e79a5','#f18f3b','#5aa155','#77b7b2','#edc958','#af7aa0','#fe9ea8','#9c7561','#bab0ac','#e0585b'];
